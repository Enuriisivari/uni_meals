...
import { CanteenSidebar as Sidebar } from "../components/CanteenSidebar";
...
export function StaffPortalPage() {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      ...
      try {
        const { data } = await api.get("/canteen-orders");
        if (Array.isArray(data)) {
          setOrders(data);
        }
      } catch {
  ...
  const handleUpdateOrderStatus = async (orderId, status) => {
    ...
    try {
      const { data } = await api.patch(`/canteen-orders/${orderId}/status`, { status });
